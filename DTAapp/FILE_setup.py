...
def setting(H_prime, H_double_prime, data):
    #a = pd.read_csv(filename, sep = "\t", skiprows = [1,2])
    final = np.c_[pd.to_numeric(data[data.columns[0]],errors='coerce') , pd.to_numeric(data[data.columns[H_prime]],errors='coerce') , pd.to_numeric(data[data.columns[H_double_prime]],errors='coerce')]
    np.savetxt("trainingData.txt", final)
   
def Voltage_list(Voltages) : 
